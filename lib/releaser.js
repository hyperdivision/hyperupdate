...
module.exports = class Releaser {
  ...
  upgrade (release, appPath, execPath, argv, cb) {
    this._getChunks(err => {
      ...
      if (process.platform === 'darwin') appPath = path.join(appPath, 'Contents')
  ...
  isDownloaded (release, cb) {
    fs.readFile(this.unpackedVersion, 'utf-8', (_, version) => {
      cb(null, version === release.version)
    })
  }
  ...
  downloadRelease (release, cb) {
    ...
    if (this.downloading) return process.nextTick(cb, new Error('Already downloading'))
    this.downloading = true

    this._getChunks(err => {
      if (cancelled || err) return ondone(err)

      fs.readFile(this.unpackedVersion, 'utf-8', (_, version) => {
        if (cancelled || version === release.version) return ondone(null, this.unpacked)
        if (!release.chunks.length) return ondone(new Error('Refusing to unpack empty release'))

        download = this.chunks.feed.download(release.chunks)

        fs.unlink(this.unpackedVersion, (err) => {
          if (cancelled || (err && err.code !== 'ENOENT')) return ondone(err)

          rimraf(this.unpacked, (err) => {
            if (cancelled || err) return ondone(err)

            stream = this.createReleaseStream(release)
            pump(stream, tar.extract(this.unpacked, { map, fs }), (err) => {
              stream = null
              if (cancelled || err) return ondone(err)

              fs.writeFile(this.unpackedVersion, release.version, (err) => {
                if (cancelled || err) return ondone(err)

                ondone(null, this.unpacked)
              })
            })

            function map (header) {
              // replace the Content folder inplace, makes perms much easier
              if (process.platform === 'darwin') header.name = header.name.replace(/^\.\//, '').replace(/^Contents\//, '')
    ...
    function ondone (err, val) {
      if (cancelled && !err) err = new Error('Cancelled')

      if (download) self.chunks.feed.undownload(download)
      download = null
    ...
    function cancel (cb) {
      ...
      if (stream) stream.destroy(new Error('Cancelled'))
      stream = null
    }
  }
