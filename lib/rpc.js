...
class HRPCServiceUpdater {
  constructor (rpc) {
    ...
    this._downloadUpdate = service.defineMethod({
      id: 4,
      requestEncoding: RPC.NULL,
      responseEncoding: RPC.NULL
    })

    this._nextUpdate = service.defineMethod({
      id: 5,
      requestEncoding: RPC.NULL,
      responseEncoding: messages.Release
    })
  }
  ...
  onRequest (context, handlers = context) {
    ...
    if (handlers.downloadUpdate) this._downloadUpdate.onrequest = handlers.downloadUpdate.bind(context)
    if (handlers.nextUpdate) this._nextUpdate.onrequest = handlers.nextUpdate.bind(context)
  }
  ...
  downloadUpdate () {
    return this._downloadUpdate.request()
  }

  downloadUpdateNoReply () {
    return this._downloadUpdate.requestNoReply()
  }

  nextUpdate () {
    return this._nextUpdate.request()
  }

  nextUpdateNoReply () {
    return this._nextUpdate.requestNoReply()
  }
}
